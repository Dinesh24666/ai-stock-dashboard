...
if is_single_search:
    ...
    scan_limit = st.sidebar.slider(
        ...
        step=25,
        help="Full universe supported. 200–500 is smooth; 1000–2000 is slower but works with caching.",
        key="scan_limit"
    )
    tickers_to_scan = all_symbols[:scan_limit]
    st.sidebar.caption(f"Universe size: {total_found} · Scanning: {scan_limit}")
else:
...
def fetch_screener_universe(ticker_list):
    """Batch download optimized for large universes (up to ~2000 stocks)."""
    if not ticker_list:
        return pd.DataFrame()
    ...
    # Adaptive chunk size: smaller for huge scans to avoid rate limits / timeouts
    if total >= 1000:
        chunk_size = 30
        sleep_between = 0.55
        period = "6mo"
    ...
    for c_idx, chunk in enumerate(chunks):
        progress_bar.progress(
            (c_idx + 1) / len(chunks),
            text=f"Scanning batch {c_idx+1}/{len(chunks)} ({min((c_idx+1)*chunk_size, total)}/{total} stocks)..."
        )
            ...
            if hist is not None and not hist.empty:
                ...
                if st.button("Generate Short-Term Swing Setup for " + selected_stock):
                    if not GEMINI_API_KEY:
                        ...
                        # Short prompt = faster + stable verdicts
...
Exit: <1 line invalidation>"""
                        with st.spinner("AI analyzing (fast)..."):
                            ...
                            error_logs = []
                            candidate_models = []
                            # Discover live models from API (handles renames like gemini-3.6-flash)
                            try:
                            ...
                            # Prefer fast flash models; put known-current names first
                            preferred = [
                                "gemini-3.6-flash",
                                "gemini-2.5-flash",
                                "gemini-2.0-flash",
                            ...
                            # Sort: preferred first, then any other discovered models
                            ordered = []
                            for p in preferred:
                            ...
                            for m in candidate_models:
                                if m not in ordered:
                                    ordered.append(m)
                            if not ordered:
                                ordered = preferred  # last-resort hard-coded list

                            for model_name in ordered[:8]:  # try at most 8
                                try:
                                    ...
                                    if res and res.text:
                                        ...
                                        if "STRONG BUY" in t_up:
                                            ...
                                            vs, vsig = 45, "🟠 WAIT (AI)"
                                        elif "AVOID" in t_up or "SELL" in t_up:
                                            vs, vsig = 18, "🔴 AVOID (AI)"
                                        else:
                            ...
                            if not success:
                                st.error("AI failed — all model attempts returned errors.")
